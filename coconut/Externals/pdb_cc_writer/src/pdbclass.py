...
class PDB:
	...
	def translate_model(self,ITcoord):
		
		for i in range(self.iter_length()):
			
			self.set_x(i,self.x()[i]+ITcoord[0])
			self.set_y(i,self.y()[i]+ITcoord[1])
			self.set_z(i,self.z()[i]+ITcoord[2])
	...
	def write_selection_multimdl(self, outfilename, selection, modelid):
		...
		fout.writelines('MODEL'+str(modelid).rjust(9)+'\n')
		for i in selection:
			serial = str(self.__serial[i])
			name = self.__name[i]
			resName = self.__resName[i]
